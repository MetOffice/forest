...
def main(argv=None):
    ...
    for dataset_name in gallery.pools.keys():
        layer = gallery.pools[dataset_name].acquire()
        if hasattr(layer, "map_view"):
            if hasattr(layer.map_view, "connect"):
                layer.map_view.connect(store)
